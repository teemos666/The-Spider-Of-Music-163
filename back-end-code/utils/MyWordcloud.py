...
def get_cloud():
    # 1、读入txt文本数据
    text = open('utils/comments.txt', "r",encoding='utf-8').read()
    ...
    # 3、初始化自定义背景图片
    image = Image.open('utils/teemos.jpg')
    graph = np.array(image)

    # 4、产生词云图
    # 有自定义背景图：生成词云图由自定义背景图像素大小决定
    wc = WordCloud(font_path="utils/zifu.ttf", background_color='white', max_font_size=50,
                   mask=graph)
    wc.generate(result)
    ...
    wc.to_file("utils/wordcloud.png")  # 按照背景图大小保存绘制好的词云图，比下面程序显示更清晰
